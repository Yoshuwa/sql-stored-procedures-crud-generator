...
`<auto-generated>` marker and `sp_CRUDGen` attribution are displayed. Tables and
...
The application passes SQL `NULL` to `@SchemaTableOrViewName`. This is the
upstream procedure's database-wide mode: it processes all eligible tables and
views without the desktop application discovering or displaying them.
