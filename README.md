...
for the selected database. It can preview or create procedures database-wide without
loading tables and views into the UI.
...
account can access, then select one to load its generated procedures. SQL authentication passwords live only in the current process and
...
1. **Preview database-wide SQL** calls `sp_CRUDGen` with
   `@GenerateStoredProcedures = 0` and `@SchemaTableOrViewName = NULL`.
...
2. When a database is selected, it reads `sys.procedures`, `sys.schemas`, and
...
5. For generation, the app calls `dbo.sp_CRUDGen` as a parameterized stored
   procedure. `@SchemaTableOrViewName` is passed as `NULL`, which tells the
   upstream generator to process every eligible table and view without loading
   those objects into the desktop UI. Every checkbox maps to its corresponding
   `@Generate...` parameter.
