...
public sealed partial class SqlServerCrudGeneratorService : ICrudGeneratorService
{
    ...
    public async Task<StoredProcedureDetails> GetStoredProcedureDetailsAsync(
        ConnectionProfile profile, StoredProcedureInfo procedure, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = """
            ...
            SELECT prm.name, TYPE_NAME(prm.user_type_id), prm.max_length,
                   prm.precision, prm.scale, prm.is_output
            FROM sys.parameters AS prm
            INNER JOIN sys.procedures AS p ON p.object_id = prm.object_id
            INNER JOIN sys.schemas AS s ON s.schema_id = p.schema_id
            WHERE s.name = @SchemaName AND p.name = @ProcedureName
            ORDER BY prm.parameter_id;
            """;
        await using var command = new SqlCommand(sql, connection);
        command.Parameters.Add("@SchemaName", SqlDbType.NVarChar, 128).Value = procedure.Schema;
        command.Parameters.Add("@ProcedureName", SqlDbType.NVarChar, 128).Value = procedure.Name;
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        if (!await reader.ReadAsync(cancellationToken) || reader.IsDBNull(0))
            throw new InvalidOperationException($"Procedure {procedure.DisplayName} was not found or its definition is encrypted.");
        ...
        return new StoredProcedureDetails(procedure, definition, parameters);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var procedure = new StoredProcedureInfo(table.Schema, table.Name + suffix, DateTime.MinValue);
            results.Add(await TestGeneratedProcedureCoreAsync(connection, procedure, cancellationToken));
        }
        return results;
    }

    public async Task<ProcedureTestResult> TestGeneratedProcedureAsync(
        ConnectionProfile profile, StoredProcedureInfo procedure, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(procedure);
        await using var connection = CreateConnection(profile);
        await connection.OpenAsync(cancellationToken);
        return await TestGeneratedProcedureCoreAsync(connection, procedure, cancellationToken);
    }

    private static async Task<ProcedureTestResult> TestGeneratedProcedureCoreAsync(
        SqlConnection connection, StoredProcedureInfo procedure, CancellationToken cancellationToken)
    {
        ...
        definitionCommand.Parameters.Add("@SchemaName", SqlDbType.NVarChar, 128).Value = procedure.Schema;
        definitionCommand.Parameters.Add("@ProcedureName", SqlDbType.NVarChar, 128).Value = procedure.Name;
        var definition = await definitionCommand.ExecuteScalarAsync(cancellationToken) as string;
        if (definition is null)
            return new ProcedureTestResult(procedure.DisplayName, false, "Procedure was not found.");
        if (!StoredProcedureInfo.IsGeneratedBySpCrudGen(definition))
            return new ProcedureTestResult(procedure.DisplayName, false, "Procedure does not contain the sp_CRUDGen marker.");
        ...
        try
        {
            ...
            refreshCommand.Parameters.Add("@name", SqlDbType.NVarChar, 776).Value = QuoteQualifiedName(procedure.Schema, procedure.Name);
            await refreshCommand.ExecuteNonQueryAsync(cancellationToken);
            await transaction.RollbackAsync(cancellationToken);
            return new ProcedureTestResult(procedure.DisplayName, true,
                "Exists, is generated by sp_CRUDGen, and SQL Server rebound it successfully.");
        }
        catch (SqlException exception)
        {
            try { await transaction.RollbackAsync(CancellationToken.None); }
            catch (InvalidOperationException) { }
            return new ProcedureTestResult(procedure.DisplayName, false, exception.Message);
        }
    }
